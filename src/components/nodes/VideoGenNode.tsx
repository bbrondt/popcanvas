...
export function VideoGenNode({ id, data, selected }: NodeProps) {
  ...
  /** Set when extension failed and we silently fell back to last-frame.
   *  Surfaces the actual Veo error in the UI so the user can see WHY
   *  extension didn't work without digging into the browser console. */
  const [extendFallbackReason, setExtendFallbackReason] = useState<string | undefined>(undefined);
  ...
  const generate = async () => {
    ...
    setError(undefined);
    setExtendFallbackReason(undefined);
      ...
      if (startingFrame?.kind === 'video-gen-extend') {
        ...
        try {
          ...
          if (looksLikeExtendSourceFailure(msg)) {
            ...
            setExtendFallbackReason(msg);
            setStatusMsg('Extension failed — falling back to last frame…');
  ...
  return (
    ...
    >
      ...
      {extendFallbackReason && !isGenerating && outputUrl && (
        <div className="border border-amber-500/40 bg-amber-500/5 px-2 py-1.5 mb-2 text-[11px] font-sans text-amber-200 rounded-sm flex items-start gap-2">
          <div className="flex-1 leading-snug">
            <div className="node-label text-amber-400 mb-0.5">⚠ this clip is NOT a real extension</div>
            <div className="text-amber-100">
              Native Veo extension failed. The clip you're seeing was generated by image-to-video from the prior clip's last frame, which is why motion / character / voice may not feel continuous.
            </div>
            <div className="mt-1 text-amber-300/80 break-all font-mono text-[10px]">
              veo error: {extendFallbackReason}
            </div>
          </div>
          <button onClick={() => setExtendFallbackReason(undefined)} className="text-amber-300 hover:text-amber-100 font-mono text-xs leading-none">
            ×
          </button>
        </div>
      )}
